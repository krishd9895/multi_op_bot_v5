...
class TourDiaryBot:
    ...
    def show_village_buttons(self, message, villages: List[str]):
            ...
            for activity in activities[year_str][month_str]:
                try:
                    ...
                    if activity_date.month == current_month and activity_date.year == current_year:
                        covered_villages.add(activity['to_village'])
                except ValueError:
                    continue
        ...
        sent = self.bot.send_message(
            message.chat.id,
            f"🏘️ Select the village you visited:{covered_text}",
            reply_markup=keyboard
        )
    ...
    def handle_settings_sethq(self, message, timeout=USER_TIMEOUT):
        ...
        if user_id in self.cancelled_users:
            ...
            return
        headquarters = message.text.strip()
    ...
    def daily_prompt(self):
        ...
        for user in users:
            try:
                ...
                keyboard.add(
                    types.InlineKeyboardButton(
                        f"🏢 {headquarters} (headquarters)", callback_data=f"daily_village_{headquarters}"
                    )
                )
    ...
    def default_activity_fallback(self):
        ...
        for user in users:
            try:
                ...
                # Get used villages from new structure
                if year_str in activities and month_str in activities[year_str]:
                    for activity in activities[year_str][month_str]:
                        try:
                            activity_date = datetime.strptime(activity['date'], '%d/%m/%Y')
                ...
                # Prepare message based on whether purpose was randomly selected
                purpose_explanation = (
                    "Since you haven't set a default purpose, I randomly selected one from this month's activities."
                    if not user.get('default_purpose') else
    ...
    def setup_handlers(self):
        ...
        def callback_query(call):
            ...
            if call.data.startswith('village_'):
                headquarters = users_collection.find_one({'user_id': user_id}).get('headquarters', 'HQ')
    ...
    def td_month_command(self, message):
        ...
        if first_name or last_name:
            ...
            user_name = 'User'
        month_name = calendar.month_name[month_filter]
        heading_text = f"TOUR DIARY OF {user_name}, {headquarters} FOR THE MONTH OF {month_name}-{year_filter}".upper()
